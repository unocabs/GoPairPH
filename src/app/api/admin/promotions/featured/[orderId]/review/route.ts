...
export async function PATCH(request: Request, { params }: { params: { orderId: string } }) {
  ...
  const supabase = createClient();
  const { data, error } = await supabase.rpc('admin_review_featured_promotion', {
  ...
  if (body.action === 'approve' || body.action === 'reject') {
    ...
    if (listing && sellerUserId) {
      try {
        ...
        if (authData.user?.email) {
          ...
          await sendEmail({
            ...
            subject: body.action === 'approve'
              ...
              : `Featured request needs review: ${listingName}`,
        ...
        console.error('[featured-promotion] seller review email failed', emailError);
      }
    }
  }
